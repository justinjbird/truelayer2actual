...
async function validateBalance(
  ...
  const tlCurrentPence =
    Math.round(tlBalance.current * 100) *
    (account.accountKind === "card" ? -1 : 1);
...
async function main(): Promise<void> {
  logger.info("Starting truelayer sync...");
  ...
  if (config.accounts.length === 0) {
    logger.warn(
      'No accounts configured. Run "npm run setup" to pair accounts.',
    );
    process.exit(0);
  }
  ...
  const connectionIds = [
    ...new Set(config.accounts.map((a) => a.connectionId)),
  ];
  ...
  try {
    for (const account of config.accounts) {
      ...
      const existing = await getActualTransactions(
      ...
      if (skipped.length > 0) {
        ...
        skipped.forEach((t) =>
          logger.debug(
            `[${account.name}] Skipped ${t.date} ${t.amount}p ${t.payee_name ?? ""}`,
          ),
        );
      }
      ...
      logger.info(
        `[${account.name}] +${result.added.length} added, ${result.updated.length} updated`,
      );

      if (result.errors && result.errors.length > 0) {
        throw new Error(
          `[${account.name}] Import errors: ${JSON.stringify(result.errors)}`,
        );
      }
